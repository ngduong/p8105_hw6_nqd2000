...
![](p8105_hw6_nqd2000_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
...
r_sqr = weather_stats %>% 
  ggplot(aes(x = r.squared, 
             fill = "lightgreen", 
             color = "lightgreen")) +                  #plot R-squared's
...
log_est = weather_stats %>% 
  ggplot(aes(x = log_beta_prod, 
             fill = "lightpurple", 
             color = "lightpurple")) +                  #plot density of log_beta_prod
  geom_density(alpha = 0.3) +
...
![](p8105_hw6_nqd2000_files/figure-gfm/unnamed-chunk-16-1.png)<!-- -->

We can get 95% CI for R-squared and
log(β̂0xβ̂1)

``` r
weather_stats %>% pull(r.squared) %>% as.vector() %>% quantile(c(0.025, 0.975))
```
...
``` r
weather_stats %>% pull(log_beta_prod) %>% as.vector() %>% quantile(c(0.025, 0.975))
```
